...
class FluxProFillTool(Tool):
    ...
    def _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage]:
        ...
        mask: File = tool_parameters.get("mask", None)
        if mask is not None:
            response = requests.get(mask.url)
            if response.status_code == 200:
                input_image = Image.open(BytesIO(response.content))
                buffered = BytesIO()
                image_format: str = data["output_format"]
                ...
                raise Exception(
                    f"Failed to fetch mask from URL: {mask.url}"
                )
        else:
            raise Exception("No mask provided for Flux Pro Fill Tool.")
