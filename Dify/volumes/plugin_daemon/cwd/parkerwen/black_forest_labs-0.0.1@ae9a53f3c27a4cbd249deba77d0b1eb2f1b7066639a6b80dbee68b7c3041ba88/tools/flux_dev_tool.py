...
class FluxDevTool(Tool):
    ...
    def _make_request(self, headers: dict, data: Any, files: Any = None, region: str = "EU1"):
        req = PreparedRequest()
    ...
    def _handle_response(self, response: Response, headers: dict, region: str):
        return self._poll_for_result(response.json().get("id"), headers, region)

    def _poll_for_result(self, id: str, headers: dict, region: str = "EU1"):
    ...
    def _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage]:
        data = {
            ...
            "steps": tool_parameters.get("steps", 28),
            "prompt_upsampling": tool_parameters.get("prompt_upsampling", True),
            "seed": tool_parameters.get("seed", None),
            "guidance": tool_parameters.get("guidance", 3),
        ...
        if image_prompt is not None:
            ...
            if response.status_code == 200:
                ...
                buffered = BytesIO()
                image_format: str = tool_parameters.get(
                    "output_format", "jpeg"
                )
        ...
        response = self._make_request(
            headers, data, files=None, region=tool_parameters.get("region", "EU1")
        )

        if response.status_code == 200:
            image_url = self._handle_response(
                response, headers, tool_parameters.get("region", "EU1")
            )
